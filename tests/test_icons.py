...
def check_icon_set(
	...
	if not main_specification:
		...
		assert len(main_specification) == len(icon.main_icon.shape_specifications)
		for index, shape in enumerate(main_specification):
			shape_specification: ShapeSpecification = (icon.main_icon.shape_specifications[index])
...
def test_no_icon_1_extra() -> None:
    ...
    """
	check_icon_set({"access": "private"}, [], [[("lock_with_keyhole", EXTRA_COLOR)]])
