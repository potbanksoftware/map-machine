...
def is_matched_tag(
	...
	if isinstance(matcher_tag_value, str) and matcher_tag_value.startswith('^'):
		matcher: Optional[re.Match] = re.match(matcher_tag_value, tags[matcher_tag_key])
		if matcher:
...
def match_location(restrictions: dict[str, str], country: str) -> bool:
	...
	if "exclude" in restrictions and country in restrictions["exclude"]:
		return False
	if (
			"include" in restrictions and restrictions["include"] != "world"
...
class Matcher(Tagged):
	...
	def __init__(self, structure: dict[str, Any], group: Optional[dict[str, Any]] = None) -> None:
	...
	def check_zoom_level(self, zoom_level: float) -> bool:
		...
		return (self.start_zoom_level is None or zoom_level >= self.start_zoom_level)
	...
	def is_matched(self, tags: Tags, country: Optional[str] = None) -> tuple[bool, dict[str, str]]:
		...
		if (
				country is not None and self.location_restrictions
				and not match_location(self.location_restrictions, country)
				):
			return False, {}

		for config_tag_key in self.tags:
			is_matched, matched_groups = is_matched_tag(config_tag_key, self.tags[config_tag_key], tags)
		...
		if self.exception:
			for config_tag_key in self.exception:
				is_matched, matched_groups = is_matched_tag(config_tag_key, self.exception[config_tag_key], tags)
				if is_matched != MatchingType.NOT_MATCHED:
					return False, {}
...
def get_shape_specifications(structure: list[Union[str, dict[str, Any]]]) -> list[dict[str, str]]:
...
class Scheme:
	...
	def __init__(self, content: dict[str, Any]) -> None:
		...
		self.way_matchers: list[WayMatcher] = ([WayMatcher(x, self)
												for x in content["ways"]] if "ways" in content else [])
		self.road_matchers: list[RoadMatcher] = ([RoadMatcher(x, self)
													for x in content["roads"]] if "roads" in content else [])
		self.area_matchers: list[Matcher] = ([Matcher(x)
												for x in content["area_tags"]] if "area_tags" in content else [])
	...
	def get_icon(
		...
		for index, matcher in enumerate(self.node_matchers):
			...
			if not matching:
				continue
			if not ignore_level_matching and not matcher.check_zoom_level(zoom_level):
				raise ValueError
			...
			if matcher.shapes:
				specifications = [self.get_shape_specification(x, extractor, groups) for x in matcher.shapes]
				main_icon = Icon(specifications)
				processed |= matcher_tags
			if matcher.over_icon and main_icon:
				specifications = [self.get_shape_specification(x, extractor) for x in matcher.over_icon]
				main_icon.add_specifications(specifications)
				processed |= matcher_tags
			if matcher.add_shapes:
				specifications = [
						self.get_shape_specification(x, extractor, color=self.get_extra_color())
		...
		returned: IconSet = IconSet(main_icon, extra_icons, default_icon, processed)
			...
			if key in tags:
				for specification in main_icon.shape_specifications:
					if (
							...
							and DirectionSet(tags[key]).is_right() != specification.shape.is_right_directed
							):
	...
	def get_style(self, tags: dict[str, Any]) -> list[LineStyle]:
		...
		for matcher in self.way_matchers:
			...
			line_style: LineStyle = LineStyle(matcher.style, matcher.parallel_offset, matcher.priority)
