...
from map_machine.geometry.vector import Line, Polyline, compute_angle, norm, turn_by_angle
...
class RoadPart:
	...
	def __init__(
		...
		self.turned: np.ndarray = norm(turn_by_angle(self.point_2 - self.point_1, np.pi / 2.0))
	...
	def update(self) -> None:
		...
		if self.left_connection is not None:
			self.right_projection = (self.left_connection + self.right_vector - self.left_vector)
		if self.right_connection is not None:
			self.left_projection = (self.right_connection - self.right_vector + self.left_vector)
...
class Intersection:
	...
	def __init__(self, parts: list[RoadPart]) -> None:
		...
		for index, part_1 in enumerate(self.parts):
			...
			if (part_1.right_connection is None and part_2.left_connection is None):
...
class Road(Tagged):
	...
	def __init__(
		...
		self.line: Polyline = Polyline([flinger.fling(node.coordinates) for node in self.nodes])
		...
		if "width:lanes" in tags:
			try:
				widths: list[float] = list(map(float, tags["width:lanes"].split('|')))
		...
		if "placement" in tags:
			...
			if value == "transition":
				...
				if lane_number > 0:
					self.placement_offset += sum(lane.get_width(self.scale) for lane in self.lanes[:lane_number])
				elif lane_number < 0:
					self.placement_offset += (DEFAULT_LANE_WIDTH * lane_number * self.scale)

				if place == "left_of":
					pass
				elif place == "middle_of":
					self.placement_offset += (self.lanes[lane_number].get_width(self.scale) * 0.5)
				elif place == "right_of":
					self.placement_offset += self.lanes[lane_number].get_width(self.scale)
				else:
					logging.error(f"Unknown placement `{place}`.")

	def get_style(self, is_border: bool, is_for_stroke: bool = False) -> dict[str, Union[int, float, str]]:
		"""Get road SVG style."""
		width: float
	...
	def draw_lanes(self, svg: Drawing, color: Color) -> None:
		...
		for index in range(1, len(self.lanes)):
			lane_offset: float = self.scale * (-self.width / 2.0 + index * self.width / len(self.lanes))
			path: Path = Path(d=self.line.get_path(self.placement_offset + lane_offset))
			style: dict[str, Any] = {
					"fill": "none",
					"stroke": color.hex,
					"stroke-linejoin": "round",
	...
	def draw_caption(self, svg: Drawing) -> None:
		...
		path: Path = svg.path(d=self.line.get_path(self.placement_offset + 3.0), fill="none")
		svg.add(path)
...
def get_curve_points(
	...
	if is_end:
		direction = -direction
	left: np.ndarray = turn_by_angle(direction, np.pi / 2.0) * (width + placement_offset)
	right: np.ndarray = turn_by_angle(direction, -np.pi / 2.0) * (width - placement_offset)
...
class Connector:
	...
	def __init__(
		...
		self.min_layer: float = min(connection[0].layer for connection in connections)
		self.max_layer: float = max(connection[0].layer for connection in connections)
...
class ComplexConnector(Connector):
	...
	def __init__(
		...
		length: float = (abs(self.road_2.width - self.road_1.width) * self.road_1.scale)
...
class SimpleIntersection(Connector):
	...
	def draw(self, svg: Drawing) -> None:
		"""Draw connection fill."""
		for road, _ in sorted(self.connections, key=lambda x: x[0].matcher.priority):
			node: OSMNode = self.road_1.nodes[self.index_1]
...
class Roads:
	...
	def draw(self, svg: Drawing, flinger: Flinger, draw_captions: bool = False) -> None:
		...
		for road in self.roads:
			if not road.is_transition:
				...
				for end in 0, -1:
					connections.append([
							connection for connection in self.nodes[road.nodes[end].id_]
							if not connection[0].is_transition
							])
				if len(connections[0]) == 1 and len(connections[1]) == 1:
					connector: Connector = ComplexConnector([connections[0][0], connections[1][0]], flinger)
		...
		for connected in self.nodes.values():
			...
			if len(connected) == 2:
				...
				if (
						road_1.width == road_2.width or index_1 not in [0, len(road_1.nodes) - 1]
		...
		for layer in sorted(layered_roads.keys()):
			roads: list[Road] = sorted(layered_roads[layer], key=lambda x: x.matcher.priority)
