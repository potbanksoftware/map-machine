...
__all__ = ["Line", "Polyline", "Segment", "compute_angle", "norm", "turn_by_angle"]
...
class Polyline:
		...
		if np.allclose(parallel_offset, 0.0):
			...
			try:
				points = (
						...
						if parallel_offset else self.points
						)
			except (ValueError, NotImplementedError):
				points = self.points

		return (
				"M " + " L ".join(f"{point[0]},{point[1]}" for point in points) +
	...
	def shorten(self, index: int, length: float) -> None:
		...
		self.points[index] = (self.points[index] + diff / np.linalg.norm(diff) * length)
...
class Line:
	...
	def get_intersection_point(self, other: "Line") -> np.ndarray:
		...
		x: float = -(self.b * other.c - other.b * self.c) / (other.a * self.b - self.a * other.b)
		y: float = -(self.a * other.c - other.a * self.c) / (other.b * self.a - self.b * other.a)
		return np.array((x, y))
...
class Segment:
	...
	def intersection(self, other: "Segment") -> Optional[list[float]]:
		...
		t: float = ((self.point_1[0] - other.point_1[0]) * (other.point_1[1] - other.point_2[1]) -
					(self.point_1[1] - other.point_1[1]) * (other.point_1[0] - other.point_2[0])) / divisor

		u: float = ((self.point_1[0] - other.point_1[0]) * (self.point_1[1] - self.point_2[1]) -
					(self.point_1[1] - other.point_1[1]) * (self.point_1[0] - self.point_2[0])) / divisor
